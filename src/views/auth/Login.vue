...
const loginHandler = async () => {
  ...
  if (isEmailEmpty && isPasswordEmpty) { formErrorMessage.value = '이메일과 비밀번호를 입력해주세요'; return }
  if (isEmailEmpty) { formErrorMessage.value = '이메일을 입력해주세요'; return }
  if (isPasswordEmpty) { formErrorMessage.value = '비밀번호를 입력해주세요'; return }
  ...
  <main class="wall min-h-screen flex items-center justify-center px-6 py-16">
    ...
    <div class="note-wrapper relative w-full max-w-sm">
      ...
      <div class="tape absolute z-20"
           style="top:-17px; left:50%; width:105px; height:30px;
